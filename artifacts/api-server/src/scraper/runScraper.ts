...
// Export so it can be called from the API route
export { main as runScraper };

// Only auto-run when executed directly (e.g. `npx tsx scraper/runScraper.ts`)
if (process.argv[1]?.endsWith("runScraper.ts") || process.argv[1]?.endsWith("runScraper.js")) {
  main().catch((err) => {
