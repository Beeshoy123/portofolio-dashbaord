...
import { fmt, fmt1, fmt2, getScoreColor } from "./portfolioMath";
...
//   ABR (Bareeq):  Money-market emergency reserve. NAV accrues daily. NO dividend
...
${buildUsdRealityCard(p, d, usdReality)}
  ...
  <div class="card dark s-2" data-view-card="health">
    ...
    <div class="wh-metrics">
      <div class="wh-metric"><span class="wh-dot" style="background:${getScoreColor(d.health.diversityScore)}"></span><span class="wh-mname" data-i18n="health.diversity">Diversity</span><div class="wh-track"><div class="wh-fill" id="wh-div" style="width:${Math.round(d.health.diversityScore)}%;background:${getScoreColor(d.health.diversityScore)}"></div></div><span class="wh-mval" id="wh-div-v">${Math.round(d.health.diversityScore)}</span></div>
      <div class="wh-metric"><span class="wh-dot" style="background:${getScoreColor(d.health.emergencyFundScore)}"></span><span class="wh-mname" data-i18n="health.ef">Emergency fund</span><div class="wh-track"><div class="wh-fill" id="wh-ef" style="width:${Math.round(d.health.emergencyFundScore)}%;background:${getScoreColor(d.health.emergencyFundScore)}"></div></div><span class="wh-mval" id="wh-ef-v">${Math.round(d.health.emergencyFundScore)}</span></div>
      <div class="wh-metric"><span class="wh-dot" style="background:${getScoreColor(d.health.yieldScore)}"></span><span class="wh-mname" data-i18n="health.yield">Yield rate</span><div class="wh-track"><div class="wh-fill" id="wh-yield-bar" style="width:${Math.round(d.health.yieldScore)}%;background:${getScoreColor(d.health.yieldScore)}"></div></div><span class="wh-mval" id="wh-yield-v">${Math.round(d.health.yieldScore)}</span></div>
      <div class="wh-metric"><span class="wh-dot" style="background:${getScoreColor(d.health.liquidityScore)}"></span><span class="wh-mname" data-i18n="health.liquidity">Liquidity</span><div class="wh-track"><div class="wh-fill" id="wh-liq-bar" style="width:${Math.round(d.health.liquidityScore)}%;background:${getScoreColor(d.health.liquidityScore)}"></div></div><span class="wh-mval" id="wh-liq-v">${Math.round(d.health.liquidityScore)}</span></div>
    </div>
    <div class="math-section" id="math-health">
      <div class="math-line"><span class="math-label" data-i18n="ml.diversity">Diversity:</span><span class="math-calc">${d.health.assetCount} holdings · HHI ${(d.health.hhi * 100).toFixed(0)}% (max ${d.health.largestAssetName} ${d.health.largestAssetPct.toFixed(0)}%)</span><span class="math-result">= ${Math.round(d.health.diversityScore)}</span></div>
      <div class="math-line"><span class="math-label" data-i18n="ml.emergency.fund">Emergency Fund:</span><span class="math-calc">${fmt(d.health.emergencyFundCurrent)} (Bareeq) ÷ ${fmt(d.settings.emergencyFundTarget)} (6 mo @ 10k/mo)</span><span class="math-result">= ${Math.round(d.health.emergencyFundScore)}</span></div>
      <div style="font-size:9.5px;color:var(--dim);margin:2px 0 6px 0;line-height:1.35">* Target = 6 months living expenses (10k/mo). Kept in Bareeq money market fund to combat inflation while maintaining daily liquidity.</div>
      <div class="math-line"><span class="math-label" data-i18n="ml.yield.rate">Yield Rate:</span><span class="math-calc">${d.health.blendedYieldPct.toFixed(1)}% blended ÷ 27% CBE risk-free benchmark</span><span class="math-result">= ${Math.round(d.health.yieldScore)}</span></div>
      <div class="math-line"><span class="math-label" data-i18n="ml.liquidity">Liquidity:</span><span class="math-calc">${fmt(d.health.liquidHoldingsValue)} liquid (${d.health.liquidityPct.toFixed(1)}% of total) · 15–25% benchmark</span><span class="math-result">= ${Math.round(d.health.liquidityScore)}</span></div>
      <div class="math-divider"></div>
...
export function buildInsights(d: Derived, lang: Lang = 'en'): string {
  ...
  if (lang === 'ar') {
    ...
    items.push(`<div class="insight-item"><div class="insight-icon">📈</div><div><div class="insight-title">صندوق الطوارئ عند ${d.health.emergencyFundPct.toFixed(0)}%</div><div class="insight-desc">رصيد بريق يغطي ${d.health.emergencyFundPct.toFixed(0)}% من هدف ${fmt(d.settings.emergencyFundTarget)} ج.م (مصاريف 6 أشهر بواقع 10 آلاف ج.م/شهر). مُحتفظ به في بريق لمقاومة التضخم مع الحفاظ على السيولة اليومية.</div></div></div>`);
    ...
    items.push(`<div class="insight-item"><div class="insight-icon">📈</div><div><div class="insight-title">Emergency Fund at ${d.health.emergencyFundPct.toFixed(0)}%</div><div class="insight-desc">Your Bareeq balance covers ${d.health.emergencyFundPct.toFixed(0)}% of your ${fmt(d.settings.emergencyFundTarget)} EGP target (6 months @ 10,000 EGP/mo). Kept in Bareeq to fight inflation while preserving liquidity.</div></div></div>`);
