...
async function tryNextData(
  ...
  if (price === null && score === null) {
    console.warn(
      `[parseStock/nextData] ${ticker}: __NEXT_DATA__ present but none of the guessed field names matched — inspect the logged keys above and fix guessFieldsFromNextData()`
    );
    return null;
  }
